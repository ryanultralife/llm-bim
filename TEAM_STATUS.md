...
| **WP-CD-ANATOMY-2** | **Claude** | `claude/grok-audit-evolution-w4umwh` | **claimed** (2 agents in flight) | remaining gap rows before Schad/INTEC rebuilds — A: `plan.py`+`construction.py` (per-discipline grid sides, split-circle detail callouts w/ register resolution, match lines incl. auto-abutting-crop, keynote squares+legend); B: `revisions.py`(new)+`schedules.py`+`sdk` (auto revision clouds from model-VCS diffs, deterministic equipment marks linking tags↔schedule) |
