...
Ops: `create_wall`/`create_rect_shell`/`place_door`/`place_window`/`create_room`/`create_slab`/`create_equipment_box`/`add_grid`/`create_note`/`set_type`/`set_phase`/`delete`, MEP place/takeoff, `csi_takeoff`, `export_pack`, …
