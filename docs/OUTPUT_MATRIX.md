...
| Industry BIM interchange | IFC4 `.ifc` | ✅ walls/slabs/doors/**IfcSpace** + FlowSegment/Fitting + **SpaceContents** + **Pset_CSIMasterFormat** | `llmbim_ifc` |
...
| Elevation DXF | DXF R12 | ✅ walls + MEP + **COLUMNS/BEAMS** + LEVELS | `export_elevation_dxf` |
| Section DXF | DXF R12 | ✅ cut plane + walls/MEP + **COLUMNS/BEAMS** + LEVELS | `export_section_dxf` |
| Section / elevation | SVG | ✅ walls/equip + MEP + **columns/beams** + storey dims | `llmbim_drawings.section` |
...
| MCP stdio | ✅ query (section/CSI/room) + takeoff + place MEP/structure + schedule kinds + pack phases + modules |
